...
<tr><td>Marginal medal value</td><td>Expected additional gold medals gained per one extra unit of capital at the current budget level — computed as the finite difference between solving the LP at <em>budget</em> and <em>budget + 0.1</em>.<br><br><strong>Positive (e.g. +0.8):</strong> each additional unit of capital is expected to buy 0.8 more golds. The frontier is still rising — there are fundable programs whose probability justifies the spend.<br><br><strong>Near zero:</strong> the frontier has flattened. Every program worth funding is already funded; extra capital cannot meaningfully improve the expected medal count at this budget level.<br><br><strong>Negative:</strong> a program that was expected to win did not, pulling the realized return below the model's forecast. This signals over-allocation — capital is being committed to programs whose performance no longer justifies the price.</td></tr>
